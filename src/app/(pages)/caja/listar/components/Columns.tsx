...
export const columns: ColumnDef<Agremiado>[] = [
    ...
    },   
    {
        accessorKey: "ahorro",
        header: ({ column }) => (
            <DataTableColumnHeader column={column} title="Total ahorrado" />
        ),
        cell: ({ row }) => {

            // const label = regiones.find((region) => region.value === row.original.region)
            const label = Number(Math.random() * 10000).toFixed(2)
            const amount = parseFloat(label)
            ...
            return (
                <span className="text-right font-black">{formatted}</span>
            )
        },
    },
    {
            ...
            return (
                <div className="flex justify-between">
                    <Button variant="outline" size="icon" asChild>
                        <Link href={`/caja/ver/${agremiado.id}`}>
                            <Eye className="h-4 w-4" />
                        </Link>
                    </Button>
                    <Button variant="outline" size="icon" asChild>
                        <Link href={`/caja/editar/${agremiado.id}`}>
